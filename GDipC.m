...
Dipstring=[];
   if(number<nf)
      for i=index(number)+1:index(number+1)-1  %第number条蛋白质所跨越的所有行
          str=[str,ex{i}];%把第number条蛋白质序列存在字符串str里
             for m=1:length(str)-6
          Dipstring=[Dipstring,' ',strcat(str(m),str(m+6))];
             end
      end
   end
   ...
   for n=1:20
       for k=1:20
           ...
           frequent_number(h)=size(strfind(Dipstring,Dipstr),2)/(size(str,2)-1); %size(str,2)-1表示该条蛋白质二联体的长度
           h=h+1;
       end
   end
